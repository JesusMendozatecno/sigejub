...
# El codigo fuente vive en installer-src\Installer.cs
...
# ─── Fuente C# ────────────────────────────────────
$csFile = Join-Path $projectDir "installer-src\Installer.cs"
if (-not (Test-Path $csFile)) {
    Write-Host "[ERROR] No se encuentra el codigo fuente: $csFile" -Foreground Red
    exit 1
}
