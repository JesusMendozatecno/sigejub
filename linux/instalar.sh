...
#  Instala PHP 8.2+, Composer, copia la variante correcta
#  (linux/mysql o linux/postgresql), configura la base de datos,
#  migra y deja listo para usar ./start.sh
...
# ─── 1) Motor de base de datos (elige la variante a copiar) ───
echo "Motor de base de datos:"
echo "  1) MySQL / MariaDB"
echo "  2) PostgreSQL"
echo "  3) SQLite (sin servidor; usa la variante mysql como base)"
read -r -p "Opcion [1]: " DBOPT
DBOPT="${DBOPT:-1}"
case "$DBOPT" in
    2) DB_ENGINE=pgsql;    VARIANT="postgresql";;
    3) DB_ENGINE=sqlite;   VARIANT="mysql";;
    *) DB_ENGINE=mysql;    VARIANT="mysql";;
esac
VARIANT_DIR="$SRC_DIR/$VARIANT"
[ -f "$VARIANT_DIR/artisan" ] || die "No se encontro la variante '$VARIANT_DIR' (falta artisan)."
ok "Motor seleccionado: $DB_ENGINE (variante: $VARIANT)"
...
if [ "$APP_DIR" = "$SRC_DIR" ] || [ "$APP_DIR" = "$VARIANT_DIR" ]; then
    die "La carpeta de instalacion no puede ser la misma donde esta el instalador ni la variante."
fi
...
# ─── 5) Copia filtrada de la variante -----------------------------------------
info "Copiando variante '$VARIANT' a $APP_DIR ..."
tar -C "$VARIANT_DIR" -cf - \
    --exclude='.git' \
...
[ -f "$APP_DIR/artisan" ] || die "La copia no dejo el archivo artisan. Revisa permisos de $APP_DIR."
...
if ! php artisan migrate --force --no-ansi >/tmp/sigejub-migrate.log 2>&1; then
    echo "[ERROR] Las migraciones fallaron. Ultimas lineas del log:"
    echo "--------------------------------------------------------"
    tail -n 40 /tmp/sigejub-migrate.log 2>/dev/null || true
    echo "--------------------------------------------------------"
    die "Revisa la configuracion de la base de datos (ver /tmp/sigejub-migrate.log)."
fi
ok "Migraciones aplicadas"

info "Ejecutando seeders..."
if ! php artisan db:seed --force --no-ansi >/tmp/sigejub-seed.log 2>&1; then
    warn "Los seeders fallaron (revisa /tmp/sigejub-seed.log; puedes reintentar con 'php artisan db:seed --force')"
else
...
# ─── 10) Accesos directos ---------------------------------------------------
chmod +x "$APP_DIR/start.sh" "$APP_DIR/detener.sh" "$APP_DIR/desinstalar.sh" "$APP_DIR/setup.sh" 2>/dev/null || true
chmod +x "$SRC_DIR/instalar.sh" 2>/dev/null || true
