...
if [ "$need_php_install" = 1 ]; then
    ...
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y php-cli php-mbstring php-xml php-curl php-gd php-zip php-intl php-sqlite3 php-mysql php-pgsql
fi
...
MISSING=""
for ext in mbstring dom curl gd zip intl fileinfo sqlite3 pdo_sqlite; do
    php -m 2>/dev/null | grep -qi "^${ext}$" || MISSING="$MISSING $ext"
done
if [ -n "$MISSING" ]; then
    warn "Faltan extensiones PHP:$MISSING. Instalando..."
    apt_update
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y php-mbstring php-xml php-curl php-gd php-zip php-intl php-sqlite3
fi
...
$COMPOSER_BIN install --no-ansi --no-interaction >/tmp/sigejub-composer.log 2>&1 \
    || die "composer install fallo. Ver /tmp/sigejub-composer.log"
