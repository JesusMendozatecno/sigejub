...
if ($p.ExitCode -eq 0) {
    ...
    # Copiar el .exe a la carpeta windows/ (donde viven los instaladores)
    $winDir = Join-Path (Split-Path -Parent $projectDir) "windows"
    if (Test-Path $winDir) {
        $winExe = Join-Path $winDir "SIGEJUB-Installer.exe"
        try {
            Copy-Item $outExe $winExe -Force
            Write-Host "[OK] Copiado a: $winExe" -Foreground Green
        } catch {
            Write-Host "[AVISO] No se pudo copiar a $winExe : $($_.Exception.Message)" -Foreground Yellow
        }
    }
